...
The expression in Eq.~\ref{eq:sum_opt_y} states that the sum of the values in a box is equal to the sum of the previous box minus the first value of the previous box plus the last value of the current box. According to that, when a box is calculated, the sum and the first value are saved in temporary variables. To calculate the next sum, the previous sum is subtracted from the temporary variable and the new value is added. The larger the box, the greater the performance gain.
