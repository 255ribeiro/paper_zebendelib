...
The \verb"dmcx2_from_p_dcca_matrix_2d()" function uses \pkg{Numpy} methods to preparate the data to apply Eq. \ref{eq:dmcx2}. The list of indexes is divided in \verb"y_index", holding an one item array wiht the index of the dependent variabel time series, and \verb"x_indexes", containing the indexes of the independent ones. The \pkg{Numpy} function \verb"np.ix_()" constructs index arrays that will index the cross product from a serie of 1D arrays as inputs.
