...
Regarding performance, the pure \proglang{Python} implementation, the \proglang{Zig} implementation and the optimized \proglang{Zig} where compared with 3 oder packages that calculates the $\rho_{DCCA}$. Researching the \proglang{R} \textbf{CRAN} repository, the \pkg{DFA}, a pure \proglang{R} implementation, and  the \pkg{DCCA}~\citep{R-DCCA},  a \proglang{R} package implemented in \proglang{Java} where found. From the \proglang{Python} \textbf{pypi} repository, a package also named \pkg{DCCA}\citep{alchieri2024lateralization}, a \proglang{Python} implementation with massive use of \pkg{Numpy} broadcasting capabilities and memory access with strides to avoid loops, was used. All those packages have $\rho_{DCCA}$ functions. Hereafter the \pkg{DCCA} \proglang{R-java} package will be referred as \pkg{DCCA-R-Java} and the \pkg{DCCA} \proglang{Python} package as \pkg{DCCA-Python} for better distinction.

The $dmc_{x}^{2}$ is a more recent coefficient, and no other package was found that calculates it besides the \pkg{Zebende} package. That alone is a big contribution of this package. The $\rho_{DCCA}$ is a fundamental step to calculate the $DMC_{x}^{2}$ and contains the most computational expensive parts. In the $DMC_{x}^{2}$ calculation, The $\rho_{DCCA}$ is calculated for, at least, a combination of 3 series two by two. With the idea to broadly compare the performance of the \pkg{Zebende} package with the others, aiming an overall order of magnitude of the packages compared speed, using the same computer and the same time window scales, the performance comparison was made using data from one of the experiments described in the \cite{RIBEIRO2025107419} paper, available at the link below:

\begin{center}
  	\url{https://tinyurl.com/zbtestdata01}
\end{center}

None of the comparison packages can calculate multiple series comparison in the same input, so the $\rho_{DCCA}$ methods where wrapped around a loop that alternate the series used by the function, using a four two by two combination of the data indexes. The \pkg{DCCA-Python} package also didn't accepts multiple inputs for the time scale, so another loop was added in that test. The results are presented in the Table \ref{tab:time_4}.
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{@{}l@{\hspace{1.0cm}}l@{\hspace{1.0cm}}l@{\hspace{1.0cm}}l@{}}
      ...
      Zebende & Zig (basic) & 59.74 & 52.7\% \\
      DCCA & Python & 447.67 &  1,243.49\% \\
      DCCA & R-Java & 2,206.3 & 6,126.17\% \\
      Zebende & Python & 3,076.23 & 8,542.83\% \\
      DFA & R & 28,818.016 & 80,049.42\% \\
      \hline
    \end{tabular}
  \end{table}
    ...
    \begin{table}[h]
      ...
      \begin{tabular}{@{}l@{\hspace{1.0cm}}l@{\hspace{1.0cm}}l@{\hspace{1.0cm}}l@{}}
        ...
        Zebende & Python & 3,761.40 & 3,085.47\% \\
        DCCA & Python & 4,675.17 &  3,871.65\% \\
        DCCA & R-Java & 4,8624.44 & 40,423.52\% \\
        DFA & R & 2,060,776.24 & 81,704,499.33\% \\
        \hline
      \end{tabular}
    \end{table}
...
The $52.7\%$ increase in performance from the basic to the optimized \proglang{Zig} implementation is a good result, showcasing the effectiveness of the optimizations. Another test was conducted, wth the same data and settings as the previous one, but getting the elapsed time inside every time scale. These results are presented in Fig.~\ref{fig:elapsed_tws}.

\begin{figure}[h!]
  \centering
  \includegraphics{
    figs/elapsed_tws.png}
  \caption{\label{fig:elapsed_tws}Elapsed Time Comparison for Different Implementations (four series)}
\end{figure}

Figure \ref{fig:elapsed_tws} shows the elapsed computation time difference from the Basic to the optimized version of the algorithm increases as the time scale increases. If the test was conducted with a bigger time series, and with larger time scales the difference would be even more significant. 


\section{Summary and discussion} \label{sec:summary}

The \pkg{Zebende} package is the first tool that calculates the $DMC_{x}^{2}$ coefficient for any number of time series. The package also calculates the $\rho_{DCCA}$, a fundamental step for the $DMC_{x}^{2}$ calculation. The package was tested with data from the \cite{RIBEIRO2025107419} paper, and the results were validated. The package was also compared with other packages that calculate the $\rho_{DCCA}$, and the results showed that the \pkg{Zebende} package is the fastest. The package is available at the \textbf{pypi}.
