...
The $66.50\%$ increase in performance from the basic to the optimized \proglang{Zig} implementation is a good result, showcasing the effectiveness of the optimizations. Another test was conducted, wth the same data and settings as the previous one, but getting the elapsed time inside every time scale. These results are presented in Fig.~\ref{fig:elapsed_tws_04}.
...
With that configuration, the optimized \proglang{Zig} version still the fastest, followed by the basic \proglang{Zig} implementation. The \pkg{Zebende} pure \proglang{Python} implementation ended in the third position, performing better than the \pkg{DCCA-Python} (4th) and the \pkg{DCCA-R-Java} (5th) packages. The lists ended with the \pkg{DFA} in the 6th position.
